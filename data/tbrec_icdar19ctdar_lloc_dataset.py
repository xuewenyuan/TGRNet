...
class TbRecICDAR19cTDaRLlocDataset(BaseDataset):

    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument('--alpha', type=float, default=10, help='the adjustment factor alpha')
        parser.add_argument('--num_cols', type=int, default=44, help='the number of columns for classification')
        parser.add_argument('--num_rows', type=int, default=88, help='the number of rows for classification')
        return parser
    ...
    def __getitem__(self, index): 
        ...
        x1 = np.min(nodes[:,[2,4,6,8]],1)
        y1 = np.min(nodes[:,[3,5,7,9]],1)
        x2 = np.max(nodes[:,[2,4,6,8]],1)
        y2 = np.max(nodes[:,[3,5,7,9]],1)
        cell_boxes = np.stack((x1,y1,x2,y2),1).astype(np.float64)
        cell_boxes = cell_boxes*im_scale+np.array([pdl,pdt]*2)
        cell_boxes = torch.from_numpy(cell_boxes).to(torch.float32)

        pred_node_csv = node_csv.replace('graph_node','pred_node')
        pred_nodes = pd.read_csv(pred_node_csv).values
        pred_x1 = np.min(pred_nodes[:,[2,4,6,8]],1)
        pred_y1 = np.min(pred_nodes[:,[3,5,7,9]],1)
        pred_x2 = np.max(pred_nodes[:,[2,4,6,8]],1)
        pred_y2 = np.max(pred_nodes[:,[3,5,7,9]],1)
        pred_boxes = np.stack((pred_x1,pred_y1,pred_x2,pred_y2),1).astype(np.float64)
        pred_boxes = pred_boxes*im_scale+np.array([pdl,pdt]*2)
        pred_boxes = torch.from_numpy(pred_boxes).to(torch.float32)
        ...
        return table_name, tb_img, cell_boxes, pred_boxes, seg_row_label, seg_col_label, seg_rc_label, cls_row_label, cls_col_label, im_scale, pdl, pdt
    ...
    def collate_fn(self, batch):
        names, images, cell_boxes, pred_boxes, seg_row_label, seg_col_label, seg_rc_label, cls_row_label, cls_col_label, im_scales, pdls, pdts = list(zip(*batch))
        ...
        return {'img_names': names, 'tb_imgs': batched_imgs, 'cell_boxes': cell_boxes, 'pred_boxes': pred_boxes, 'seg_row_label': batched_seg_row_label, \
