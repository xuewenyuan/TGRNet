...
class TbrecBboxSegModel(BaseModel):
    """
    """
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['seg_row', 'seg_col', 'seg_rc']

        self.model_names = ['Backbone','CellBboxSeg']
        ...
        #                                            self.opt.load_width, self.device, self.opt.distributed, self.opt.gpu)
        ...
        if self.isTrain:
            ...
            self.optimizer = torch.optim.Adam(itertools.chain(self.netBackbone.parameters(),\
                            self.netCellBboxSeg.parameters()), lr=self.opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        self.seg_row_score, self.seg_col_score, self.seg_rc_score, self.cell_pred_boxes = self.netCellBboxSeg(inter_feat, self.table_names)
        #self.seg_row_score, self.seg_col_score, self.cell_pred_boxes = self.netCellBboxSeg(inter_feat)
        #if self.isTrain:
        #    self.cls_row_score, self.cls_col_score, self.cls_inds = self.netCellLlocPre(inter_feat, None, \
        #                                                            self.im_scales, self.pdls, self.pdts, self.cell_gt_boxes)
        #else:
        #    self.cls_row_score, self.cls_col_score, _ = self.netCellLlocPre(inter_feat, None, \
        #                                                            self.im_scales, self.pdls, self.pdts, self.cell_gt_boxes)
