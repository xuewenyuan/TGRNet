...
class TbRecModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['seg_row', 'seg_col', 'seg_rc', 'cls_row', 'cls_col']

        self.model_names = ['Backbone','CellBboxSeg', 'CellLlocPre']
        ...
        if self.isTrain:
            ...
            self.criterionLloc = torch.nn.CrossEntropyLoss().to(self.device)
            ...
            self.optimizer = torch.optim.Adam(itertools.chain(self.netBackbone.parameters(), self.netCellBboxSeg.parameters(),\
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            self.cls_row_score, self.cls_col_score, _ = self.netCellLlocPre(inter_feat, self.cell_pred_boxes, \
                                                                    self.im_scales, self.pdls, self.pdts)
